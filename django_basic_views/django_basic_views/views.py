...
def current_date(request):
    ...
    pretty_date = today_time.strftime('Today is %w, %B %Y')
    return HttpResponse(pretty_date)
...
def my_age(request, year, month, day):
    ...
    """
    my_birth = datetime(year=year, month=month, day=day)
    diff_year = datetime.now() - my_birth
    age_str = 'Your age is {} years old'.format(int(diff_year.days/365))
    return HttpResponse(age_str)
...
def profile(request):
    ...
    """
    context = {'my_name': "Jack", 'my_age': 30}
